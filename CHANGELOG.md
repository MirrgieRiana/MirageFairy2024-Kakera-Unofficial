...
カバンのレシピを変更。
ツール効果の自己修繕は、オフハンドでも機能するようになった。
ハイメヴィスカレンガのレシピを変更。
妖精の木の涙からスライムボールを作るレシピを追加。
- 幻花ファントムに局所真空崩壊への耐性を追加。
- カバンのツールチップは同名アイテムをまとめて集計するようになった。
みらなぎの杖に進捗を追加。
ガラスツールを追加。
- エンチャント「破断の呪い」を追加。
範囲採掘の効果を側面方向・手前・奥に分割。
アイテムの設置がリーチアトリビュートを無視するバグを修正。
...
素材のアイテムタグを修正。
デバッグアイテム「Dump Magic Plant Environments」を追加。
デバッグアイテム「Dump Magic Plant Traits」を追加。
磁鉄鉱製ツールは、粘着採掘と撃破時収集効果の片方のみを持つようになった。
妖精の木の涙製ツールは、粘着採掘と撃破時収集効果の片方のみを持つようになった。

涙の剣は範囲採掘効果の代わりに範囲ダメージ増加IIIを持つようになった。
- 変更: 範囲採掘効果は経路探索により接続しないブロック二は無効になった。
一括採掘効果、一括伐採効果、範囲採掘効果の挙動を調整。

以下のツールを追加。
  - 「涙の斧」
  - 「涙のクワ」

妖精の木の涙製ツールの日本語版ポエムを変更。

特性「突然変異」を追加。
- 交配判定時に、規定の確率で地形生成時の特性ビット改変を適用する。
いくつかのクライアントサイドで光量を判定する部分を修正。

確率を持つ特性効果における特性条件係数の適用方法を変更。
- 確率ではなく、所持特性の強さの部分にかかるようになった。
特性リストUIで条件のツールチップが出ないバグを修正。

植えられた魔法植物から特性リストUIを出した場合の判定位置を修正。
