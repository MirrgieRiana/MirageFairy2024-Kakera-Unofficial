...
interface FairyToolItem<I : Item> {
    val fairyToolSettings: FairyToolSettings<I>
}


// Impl

fun <I> getMiningSpeedMultiplierImpl(item: I, @Suppress("UNUSED_PARAMETER") stack: ItemStack, state: BlockState): Float where I : Item, I : FairyToolItem<I> {
    return when {
        item.fairyToolSettings.superEffectiveBlocks.any { state.isOf(it) } -> item.fairyToolSettings.toolMaterialCard.toolMaterial.miningSpeedMultiplier * 10F
        item.fairyToolSettings.effectiveBlocks.any { state.isOf(it) } -> item.fairyToolSettings.toolMaterialCard.toolMaterial.miningSpeedMultiplier
        item.fairyToolSettings.effectiveBlockTags.any { state.isIn(it) } -> item.fairyToolSettings.toolMaterialCard.toolMaterial.miningSpeedMultiplier
        else -> 1.0F
    }
}

fun <I> isSuitableForImpl(item: I, state: BlockState): Boolean where I : Item, I : FairyToolItem<I> {
    val itemMiningLevel = item.fairyToolSettings.toolMaterialCard.toolMaterial.miningLevel
    return when {
        ...
        itemMiningLevel < MiningLevels.STONE && state.isIn(BlockTags.NEEDS_STONE_TOOL) -> false
            ...
            item.fairyToolSettings.superEffectiveBlocks.any { state.isOf(it) } -> true
            item.fairyToolSettings.effectiveBlocks.any { state.isOf(it) } -> true
            item.fairyToolSettings.effectiveBlockTags.any { state.isIn(it) } -> true
            else -> false
        }
    }
}

fun <I> postMineImpl(item: I, stack: ItemStack, world: World, state: BlockState, pos: BlockPos, miner: LivingEntity) where I : Item, I : FairyToolItem<I> {
    if (item.fairyToolSettings.areaMining) run fail@{
    ...
    if (item.fairyToolSettings.mineAll) run fail@{
        if (world.isClient) return@fail
    ...
    if (item.fairyToolSettings.cutAll) run fail@{
        if (world.isClient) return@fail
...
fun <I> inventoryTickImpl(item: I, stack: ItemStack, world: World, entity: Entity, @Suppress("UNUSED_PARAMETER") slot: Int, @Suppress("UNUSED_PARAMETER") selected: Boolean) where I : Item, I : FairyToolItem<I> {
    val selfMending = item.fairyToolSettings.selfMending
...
fun <I> overrideEnchantmentLevelImpl(item: I, enchantment: Enchantment, @Suppress("UNUSED_PARAMETER") itemStack: ItemStack, oldLevel: Int): Int where I : Item, I : FairyToolItem<I> {
    if (item.fairyToolSettings.silkTouch) {
        if (enchantment == Enchantments.SILK_TOUCH) return oldLevel atLeast 1
    }
    return oldLevel
}

fun <I> convertItemStackImpl(item: I, itemStack: ItemStack): ItemStack where I : Item, I : FairyToolItem<I> {
