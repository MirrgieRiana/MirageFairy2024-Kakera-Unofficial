...
object FermentationBarrelReiCategoryCard : ReiCategoryCard<FermentationBarrelReiCategoryCard.Display>("fermentation_barrel", "Fermentation Barrel", "発酵樽") {
    ...
    class Display(val recipe: FermentationBarrelRecipe) : BasicDisplay(
        listOf(
            recipe.input1.matchingStacks.map { it.copyWithCount(recipe.inputCount1).toEntryStack() }.toEntryIngredient(),
            recipe.input2.matchingStacks.map { it.copyWithCount(recipe.inputCount2).toEntryStack() }.toEntryIngredient(),
        ),
        listOf(
