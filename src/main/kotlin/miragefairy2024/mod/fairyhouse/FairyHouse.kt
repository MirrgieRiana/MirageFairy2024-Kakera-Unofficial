...
class FairyHouseBlockEntity(pos: BlockPos, state: BlockState) : AbstractFairyHouseBlockEntity<FairyHouseBlockEntity>(FairyHouseCard, pos, state) {
    ...
    override fun tick(world: World, pos: BlockPos, state: BlockState) {
        ...
        if (folia < 1000) {
            if (world.time >= lastCollectTime + 200L) {
...
class FairyHouseScreenHandler(arguments: Arguments) : AbstractFairyHouseScreenHandler<FairyHouseBlockEntity>(FairyHouseCard, arguments)
