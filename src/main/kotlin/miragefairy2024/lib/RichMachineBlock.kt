...
abstract class RichMachineBlock(private val arguments: Arguments) : Block(arguments.settings), BlockEntityProvider {

    interface Arguments {
        val settings: Settings
        val blockEntityType: BlockEntityType<out RichMachineBlockEntity>
    }


    // Block Entity

    abstract override fun createBlockEntity(pos: BlockPos, state: BlockState): RichMachineBlockEntity
    ...
    override fun onPlaced(world: World, pos: BlockPos, state: BlockState, placer: LivingEntity?, itemStack: ItemStack) {
        if (itemStack.hasCustomName()) {
            val blockEntity = world.getBlockEntity(pos) as? RichMachineBlockEntity ?: return
    ...
    override fun onStateReplaced(state: BlockState, world: World, pos: BlockPos, newState: BlockState, moved: Boolean) {
        if (!state.isOf(newState.block)) {
            val blockEntity = world.getBlockEntity(pos) as? RichMachineBlockEntity
    ...
    override fun <T : BlockEntity> getTicker(world: World, state: BlockState, type: BlockEntityType<T>): BlockEntityTicker<T>? {
        return if (world.isClient) {
            checkType(type, arguments.blockEntityType) { world2, pos, state2, blockEntity ->
    ...
    override fun onUse(state: BlockState, world: World, pos: BlockPos, player: PlayerEntity, hand: Hand, hit: BlockHitResult): ActionResult {
        ...
        val blockEntity = world.getBlockEntity(pos) as? RichMachineBlockEntity ?: return ActionResult.CONSUME
    ...
    override fun getComparatorOutput(state: BlockState, world: World, pos: BlockPos): Int {
        val blockEntity = world.getBlockEntity(pos) as? RichMachineBlockEntity ?: return 0
