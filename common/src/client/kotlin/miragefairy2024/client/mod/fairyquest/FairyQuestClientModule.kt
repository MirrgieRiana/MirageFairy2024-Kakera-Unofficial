    ...
    RenderItemHandler.listeners += RenderItemHandler { stack, renderMode, leftHanded, matrices, vertexConsumers, light, overlay, model ->
        ...
        matrices.stack {
            ...
            matrices.stack {
                ...
                val resultModel = Minecraft.getInstance().modelManager.getModel(ModelIdentifier(ResourceLocation.fromNamespaceAndPath("minecraft", "nether_portal"), "axis=x"))
                Minecraft.getInstance().itemRenderer.render(Items.DIRT.createItemStack(), ModelTransformationMode.GUI, false, matrices, vertexConsumers, light, overlay, resultModel)
            }
            ...
            if (recipe != null) {
                matrices.stack {
                    ...
                    Minecraft.getInstance().itemRenderer.render(recipe.icon(), ModelTransformationMode.GUI, false, matrices, vertexConsumers, light, overlay, resultModel)
                }
                matrices.stack {
                    matrices.translate(0F, 1F / 16F, -0.02F)
                    matrices.scale(0.45F, 0.45F, 0.01F)
